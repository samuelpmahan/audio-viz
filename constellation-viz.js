...
export class ConstellationViz {
    constructor() {
        ...
        this.CONNECTION_DISTANCE = 18;
        
        this.particles = [];
        this.palettePhase = 0;
        
        // FIX: Track current angle separately so we can add to it smoothly
        this.currentAngle = 0; 
    ...
    animate(metrics) {
        ...
        // Instead of (Time * Speed), we do (Angle += Speed)
        // Base speed 0.002, plus extra speed from Bass
        const speed = 0.002 + (metrics.bass * 0.02);
        this.currentAngle += speed;

        this.camera.position.x = Math.sin(this.currentAngle) * 60;
        this.camera.position.z = Math.cos(this.currentAngle) * 60;
        ...
        this.particles.forEach(p => {
            ...
            const targetScale = p.baseScale + (intensity * 1.5);
    ...
    updateConnections(bassLevel) {
        ...
        const distThreshold = this.CONNECTION_DISTANCE + (bassLevel * 5);
